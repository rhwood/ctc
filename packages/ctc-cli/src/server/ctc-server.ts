...
export class CtcServer {
  ...
  readonly ipcServer = net.createServer((socket: net.Socket | JsonSocket) => {
    ...
    socket.on('message', (message: any) => {
      if (message.command !== undefined) {
        try {
          const response = this.ipcOnCommand(message.command, message.data)
          if (response !== undefined) {
            (socket as JsonSocket).sendMessage(response, () => { /* do nothing */ })
          }
        } catch (error) {
          (socket as JsonSocket).sendError(error, () => { /* do nothing */ })
        }
      }
    })
  })
  ...
  start() {
    ...
    if (this.project.config.http.port) {
      this.httpServer.listen(this.project.config.http.port, this.project.config.http.hostname, () => {
        const protocol = this.project.config.http.secure ? 'https' : 'http'
        const url = `${protocol}://${this.project.config.http.hostname}:${this.project.config.http.port}/`
    ...
    process.on('SIGTERM', () => {
      this.stop('Exiting on SIGTERM')
    })
    process.on('SIGINT', () => {
      this.stop('Exiting on SIGINT')
    })
    process.on('SIGHUP', () => {
      this.stop('Exiting on SIGHUP')
    })
    process.on('SIGBREAK', () => {
      this.stop('Exiting on SIGBREAK')
    })
  }
  ...
  cachePID(remove?: boolean | undefined) {
    const cache = Path.join(this.config.cacheDir, `pid-${process.pid}.json`)
    const pid: PID = {
      pid: process.pid,
    ...
    try {
      ...
      log.error('ERROR writing PID', '%s', error)
    }
  }
